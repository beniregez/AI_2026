...
\section{}
    \begin{enumerate}[a)]
        \item A state space that contains cycles but where tree search is still guaranteed to terminate. \\=> No, because tree search has no dublicate elimination, so the algorithm would not terminate in that case.
        \item  A state space where the optimal expansion order for graph search results in fewer expansions than the optimal expansion order for tree search \\
    \end{enumerate}
\section{}
